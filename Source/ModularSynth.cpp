...
ModularSynth::ModularSynth()
        : mMoveModule(nullptr), mIsMousePanning(false), mGlobalRecordBuffer(nullptr), mAudioPaused(false),
          mIsLoadingState(false), mClickStartX(INT_MAX), mClickStartY(INT_MAX), mWantReloadInitialLayout(false),
          mHeldSample(nullptr), mConsoleListener(nullptr), mUserPrefsEditor(nullptr), mLastClickedModule(nullptr),
          mMinimap(nullptr), mInitialized(false), mRecordingLength(0), mGroupSelectContext(nullptr),
          mResizeModule(nullptr), mShowLoadStatePopup(false), mLastSaveTime(-9999), mHasDuplicatedDuringDrag(false),
          mHasAutopatchedToTargetDuringDrag(false), mFrameRate(0), mQuickSpawn(nullptr),
          mScheduledEnvelopeEditorSpawnDisplay(nullptr), mFrameCount(0), mIsLoadingModule(false),
          mLastClapboardTime(-9999), mPixelRatio(1) {
    mConsoleText[0] = 0;
...
void ModularSynth::Setup(juce::AudioDeviceManager *globalAudioDeviceManager,
                         juce::AudioFormatManager *globalAudioFormatManager, juce::Component *mainComponent,
    ...
    if (fabsf(sBackgroundR - UserPrefs.background_r.GetDefault()) < .001f &&
        fabsf(sBackgroundG - UserPrefs.background_g.GetDefault()) < .001f &&
        fabsf(sBackgroundB - UserPrefs.background_b.GetDefault()) < .001f && time.getMonth() + 1 == 10 &&
    ...
    for (int i = 0; i < JUCEApplication::getCommandLineParameterArray().size(); ++i) {
        ...
        if (specified.endsWith(".json")) {
            ...
            if (!juce::File(ofToDataPath(filename)).existsAsFile())
                TheSynth->SetFatalError(
                        "couldn't find command-line-specified userprefs file at " + ofToDataPath(filename));
            break;
        }
    }
...
void ModularSynth::Poll() {
...

    {
        ...
        if (mIsLoadingState) {
            ...
            else if (dynamic_cast<FloatSlider *>(gHoveredUIControl) != nullptr &&
                     dynamic_cast<FloatSlider *>(gHoveredUIControl)->GetModulator() != nullptr &&
                     dynamic_cast<FloatSlider *>(gHoveredUIControl)->GetModulator()->Active())
...
void ModularSynth::Draw(void *vg) {
    ...
    if (ScriptModule::sBackgroundTextString != "") {
        ...
        DrawTextBold(ScriptModule::sBackgroundTextString, ScriptModule::sBackgroundTextPos.x,
                     ScriptModule::sBackgroundTextPos.y + ScriptModule::sBackgroundTextSize,
                     ScriptModule::sBackgroundTextSize);
        ofPopStyle();
    }

    if (UserPrefs.draw_background_lissajous.Get())
        DrawLissajous(mGlobalRecordBuffer, 0, 0, ofGetWidth(), ofGetHeight(), sBackgroundLissajousR,
                      sBackgroundLissajousG, sBackgroundLissajousB);
    ...
    if (mGroupSelectContext != nullptr) {
        ...
        ofRect(mClickStartX, mClickStartY, GetMouseX(&mModuleContainer) - mClickStartX,
               GetMouseY(&mModuleContainer) - mClickStartY);
        ofPopStyle();
    }
    ...
    if (mNextDrawTooltip != "") {
        ...
        bool hasValidHoveredControl = gHoveredUIControl && gHoveredUIControl->GetModuleParent() &&
                                      !gHoveredUIControl->GetModuleParent()->IsDeleted() &&
                                      std::string(gHoveredUIControl->Name()) != "enabled";

        if (gHoveredModule && (!hasValidHoveredControl || (gHoveredUIControl != nullptr &&
                                                           gHoveredUIControl->GetModuleParent() != gHoveredModule))) {
            if (gHoveredModule == mQuickSpawn) {
                std::string name = mQuickSpawn->GetHoveredModuleTypeName();
                ofStringReplace(name, " " + std::string(ModuleFactory::kEffectChainSuffix),
                                "");   //strip this suffix if it's there
            ...
            } else if (GetMouseY(&mModuleContainer) < gHoveredModule->GetPosition().y &&
                       gHoveredModule->HasTitleBar())  //this means we're hovering over the module's title bar
            {
    ...
    if (tooltip != "" && tooltipContainer != nullptr) {
        ...
        float padding = 3;
        ofRectangle rect(bounds[0] - padding, bounds[1] - padding, bounds[2] - bounds[0] + padding * 2,
        ...
        gFont.DrawStringWrap(tooltip, fontSize, tooltipPos.x + (onscreenRectX - rect.x),
                             tooltipPos.y + (onscreenRectY - rect.y), maxWidth);
    ...
    if (mSpaceMouseInfo.mTwist != 0) {
        ...
        ofSetLineWidth(1.5f);
        ofCircle(centerX + sin(mSpaceMouseInfo.mTwist * M_PI) * 20, centerY + cos(mSpaceMouseInfo.mTwist * M_PI) * 20,
                 3);
        ofCircle(centerX + sin(mSpaceMouseInfo.mTwist * M_PI + M_PI) * 20,
                 centerY + cos(mSpaceMouseInfo.mTwist * M_PI + M_PI) * 20, 3);
    }
    ofSetLineWidth(3);
...
void ModularSynth::KeyPressed(int key, bool isRepeat) {
    ...
    if (gHoveredUIControl &&
        ...
        } else if (key != ' ' && key != OF_KEY_TAB && key != '`' && key < CHAR_MAX &&
                   juce::CharacterFunctions::isPrintable((char) key)) {
    ...
    key = KeyToLower(
            key);  //now convert to lowercase because everything else just cares about keys as buttons (unmodified by shift)
...
float ModularSynth::GetMouseX(ModuleContainer *context, float rawX /*= FLT_MAX*/) {
    return ((rawX == FLT_MAX ? mMousePos.x : rawX) + UserPrefs.mouse_offset_x.Get()) / context->GetDrawScale() -
           context->GetDrawOffset().x;
}

float ModularSynth::GetMouseY(ModuleContainer *context, float rawY /*= FLT_MAX*/) {
    return ((rawY == FLT_MAX ? mMousePos.y : rawY) + UserPrefs.mouse_offset_y.Get()) / context->GetDrawScale() -
           context->GetDrawOffset().y;
}
...
bool ModularSynth::ShouldShowGridSnap() const {
    return (mMoveModule || (!mGroupSelectedModules.empty() && IsMouseButtonHeld(1))) &&
           (GetKeyModifiers() & kModifier_Command);
}
...
void ModularSynth::MouseMoved(int intX, int intY) {
    ...
    if (mMoveModule) {
        ...
        if (ShouldShowGridSnap()) {
            ...
            newY = round((newY - mMoveModule->TitleBarHeight()) / UserPrefs.grid_snap_size.Get()) *
                   UserPrefs.grid_snap_size.Get() + mMoveModule->TitleBarHeight();
        }
                ...
                for (auto *patchCableSource: module->GetPatchCableSources()) {
                    if (patchCableSource->TestHover(x, y)) {
                        ...
                        if (patchCableSource->GetPatchCables().size() == 0) {
                            ...
                            break;
                        } else if (patchCableSource->GetPatchCables().size() == 1 &&
                                   patchCableSource->GetTarget() != mMoveModule &&
                ...
                if (!mHasAutopatchedToTargetDuringDrag) {
                    for (auto *patchCableSource: mMoveModule->GetPatchCableSources()) {
                        if (patchCableSource && patchCableSource->GetTarget() == nullptr && module->HasTitleBar()) {
                            ofRectangle titleBarRect(module->GetPosition().x,
                                                     module->GetPosition().y - IDrawableModule::TitleBarHeight(),
                                                     module->IClickable::GetDimensions().x,
                                                     IDrawableModule::TitleBarHeight());
                            if (titleBarRect.contains(patchCableSource->GetPosition().x,
                                                      patchCableSource->GetPosition().y)) {
                                patchCableSource->FindValidTargets();
    ...
    if (gHoveredUIControl && changed) {
        if (!gHoveredUIControl->IsMouseDown()) {
            ...
            if (x < uiX - kHoverBreakDistance || y < uiY - kHoverBreakDistance || x > uiX + w + kHoverBreakDistance ||
                y > uiY + h + kHoverBreakDistance || //moved far enough away from ui control
                (y < gHoveredUIControl->GetModuleParent()->GetPosition().y && uiY >
                                                                              gHoveredUIControl->GetModuleParent()->GetPosition().y)) //hovering over title bar (and it's not the enable/disable checkbox)
...
void ModularSynth::MouseDragged(int intX, int intY, int button, const juce::MouseInputSource &source) {
            ...
            for (auto *cableSource: module->GetPatchCableSources()) {
                for (auto *cable: cableSource->GetPatchCables()) {
                    if (VectorContains(dynamic_cast<IDrawableModule *>(cable->GetTarget()), mGroupSelectedModules)) {
                        cableSource->SetPatchCableTarget(cable,
                                                         oldToNewModuleMap[dynamic_cast<IDrawableModule *>(cable->GetTarget())],
                                                         false);
                    }
                }
            }
    ...
    if (mGroupSelectContext != nullptr) {
        ...
        ofRectangle rect = ofRectangle(ofPoint(MIN(mClickStartX, x), MIN(mClickStartY, y)),
                                       ofPoint(MAX(mClickStartX, x), MAX(mClickStartY, y)));
        if (rect.width > 10 || rect.height > 10) {
            mGroupSelectContext->GetModulesWithinRect(rect, mGroupSelectedModules);
            if (mGroupSelectedModules.size() > 0) {
                for (int i = (int) mGroupSelectedModules.size() - 1;
                     i >= 0; --i) //do this backwards to preserve existing order
        ...
        if (ShouldShowGridSnap()) {
            newX = round(newX / UserPrefs.grid_snap_size.Get()) * UserPrefs.grid_snap_size.Get();
            newY = round((newY - mLastClickedModule->TitleBarHeight()) / UserPrefs.grid_snap_size.Get()) *
                   UserPrefs.grid_snap_size.Get() + mLastClickedModule->TitleBarHeight();
        }
...
void ModularSynth::MousePressed(int intX, int intY, int button, const juce::MouseInputSource &source) {
    ...
    if (gHoveredUIControl != nullptr &&
        ...
        !gHoveredUIControl->GetModuleParent()->IsHoveringOverResizeHandle() &&
        !IUIControl::WasLastHoverSetViaTab() &&
        mGroupSelectedModules.empty() &&
        ...
        ofVec2f controlClickPos(GetMouseX(gHoveredUIControl->GetModuleParent()->GetOwningContainer()),
                                GetMouseY(gHoveredUIControl->GetModuleParent()->GetOwningContainer()));
        ...
        if (clickedModule == nullptr) {
            if (rightButton) {
                ...
                if (GetModuleAtCursor(7, 7) || GetModuleAtCursor(-7, 7) || GetModuleAtCursor(7, -7) ||
                    GetModuleAtCursor(-7, -7))
...
void ModularSynth::MouseScrolled(float x, float y, bool canZoomCanvas) {
    ...
    if (IsKeyHeld(' ') || (GetModuleAtCursor() == nullptr && gHoveredUIControl == nullptr)) {
        ...
        if (floatSlider && floatSlider->GetModulator() && floatSlider->GetModulator()->Active() &&
            floatSlider->GetModulator()->CanAdjustRange()) {
...
void ModularSynth::CheckClick(IDrawableModule *clickedModule, int x, int y, bool rightButton) {
    ...
    if (modulePosY < 0 && clickedModule != TheTitleBar && (!clickedModule->HasEnableCheckbox() || modulePosX > 20) &&
        modulePosX < moduleRect.width - 15)
            ...
            if (oversampling == 1) {
                ...
                for (int sampleIndex = 0; sampleIndex < gBufferSize / oversampling; ++sampleIndex) {
                    ...
                    for (int subsample = 0; subsample < oversampling; ++subsample) {
                        output[i][sampleIndex] +=
                                mOutputBuffers[i][sampleIndex * oversampling + subsample] / oversampling;
                    }
                }
            }
        }
...
IDrawableModule *ModularSynth::CreateModule(const ofxJSONElement &moduleInfo) {
    ...
    try {
        ...
        try {
            ...
            LogEvent("Couldn't find referenced module \"" + e.mSearchName + "\" when loading \"" +
                     moduleInfo["name"].asString() + "\"", kLogEventType_Error);
        }
    }
...
void ModularSynth::SetUpModule(IDrawableModule *module, const ofxJSONElement &moduleInfo) {
    ...
    try {
        ...
        LogEvent("Couldn't find referenced module \"" + e.mSearchName + "\" when setting up \"" +
                 moduleInfo["name"].asString() + "\"", kLogEventType_Error);
    }
}
...
IDrawableModule *ModularSynth::DuplicateModule(IDrawableModule *module) {
    ...
    newModule->SetName(
            module->Name()); //temporarily rename to the same as what we duplicated, so we can load state properly
...
void ModularSynth::SaveLayoutAsPopup() {
    FileChooser chooser("Save current layout as...", File(ofToDataPath("layouts/newlayout.json")), "*.json", true,
                        false, GetFileChooserParent());
...
void ModularSynth::SaveStatePopup() {
    FileChooser chooser("Save current state as...",
                        File(ofToDataPath(ofGetTimestampString("savestate/%Y-%m-%d_%H-%M.bsk"))), "*.bsk", true, false,
                        GetFileChooserParent());
    if (chooser.browseForFileToSave(true))
...
void ModularSynth::OnConsoleInput() {
    ...
    if (tokens.size() > 0) {
        if (tokens[0] == "") {
            ...
            ofLog() << "pos:(" << mMainComponent->getTopLevelComponent()->getPosition().x << ", "
                    << mMainComponent->getTopLevelComponent()->getPosition().y << ") size:(" << ofGetWidth() << ", "
                    << ofGetHeight() << ")";
        } else if (tokens[0] == "getmouse") {
            ofLog() << "mouse pos raw:(" << mMousePos.x << ", " << mMousePos.y << ") " << "   mouse pos canvas:("
                    << GetMouseX(&mModuleContainer) << ", " << GetMouseY(&mModuleContainer) << ")";
            ...
            IDrawableModule *module = SpawnModuleOnTheFly(mConsoleText, GetMouseX(&mModuleContainer) + grabOffset.x,
                                                          GetMouseY(&mModuleContainer) + grabOffset.y);
...
namespace {
    ...
    public:
        int compareElements(juce::File first, juce::File second) {
            return (first.getCreationTime() < second.getCreationTime()) ? 1 : ((first.getCreationTime() ==
                                                                                second.getCreationTime()) ? 0 : -1);
        }
    };
}
...
IDrawableModule *ModularSynth::SpawnModuleOnTheFly(std::string moduleName, float x, float y, bool addToContainer) {
    ...
    try {
        ...
        LogEvent("Error spawning \"" + moduleName + "\" on the fly, couldn't find \"" + e.mSearchName + "\"",
